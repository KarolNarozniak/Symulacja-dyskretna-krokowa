...
@dataclass
class Samolot:
    """Reprezentacja samolotu. Parametry: id, kategoria, czasy przylotu/lądowania/odlotu."""
    id: int
    kategoria: int
    czas_przylotu: int
...
class Airport:
    ...
    def _zaplanuj_nastepny_przylot(self) -> None:
        ...
        raw = random.expovariate(1.0 / max(0.0001, self.arrival_interval))
    ...
    def _losuj_czas_ladowania(self, kategoria: int) -> int:
        ...
        val = random.gauss(self.landing_duration, 1.5)
    ...
    def _rozpocznij_ladowanie(self) -> None:
        ...
        print(f"{self.env.now}: Rozpoczecie ladowania id={sam.id}, kat={sam.kategoria}, czas={czas_l}, koniec={self.pas_zajety_do}")
    ...
    def arrival(self) -> None:
        ...
        if self.env.now == self.arrival_time:
            ...
            self.in_the_air += 1
            print(f"{self.env.now}: Przylot id={sam.id}, kat={sam.kategoria}. W powietrzu: {self.in_the_air}")
    ...
    def landing(self) -> None:
        ...
        if self.aktualny_ladujacy is not None and self.env.now == self.pas_zajety_do:
            ...
            print(f"{self.env.now}: Wyladowanie id={sam.id}. Na płycie: {self.on_the_ground}")
    ...
    def departure(self) -> None:
        ...
        if self.kolejka_na_plycie and self.kolejka_na_plycie[0].czas_odlotu_zaplanowany == self.env.now:
            ...
            print(f"{self.env.now}: Odlot id={sam.id}. Na płycie: {self.on_the_ground}")
    ...
    def _collect_stats(self) -> None:
        ...
        self.hist_kolejki_powietrze.append(len(self.kolejka_w_powietrzu) + (1 if self.aktualny_ladujacy else 0))
    ...
    def podsumuj(self) -> None:
        ...
        plt.legend()
        plt.tight_layout()
        plt.savefig("kolejki_w_czasie.png")
        print("Zapisano: kolejki_w_czasie.png")
        ...
        plt.legend()
        plt.tight_layout()
        plt.savefig("hist_czasy_oczekiwania.png")
        print("Zapisano: hist_czasy_oczekiwania.png")


if __name__ == "__main__":
    print("Start Airport z Zadania 1")
    env = simpy.Environment()
